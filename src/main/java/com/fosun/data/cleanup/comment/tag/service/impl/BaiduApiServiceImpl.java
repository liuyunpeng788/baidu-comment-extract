...
import com.fosun.data.cleanup.comment.tag.dto.db.CommentTagPo;
import com.fosun.data.cleanup.comment.tag.dto.db.ShopClassPo;
import com.fosun.data.cleanup.comment.tag.dto.db.ShopInfoPo;
import com.fosun.data.cleanup.comment.tag.dto.mongodb.MallCommentInfoPo;
import com.fosun.data.cleanup.comment.tag.dto.mongodb.MongoCommentTagInfoPo;
import com.fosun.data.cleanup.comment.tag.dto.mongodb.ShopCommentInfoPo;
import com.fosun.data.cleanup.comment.tag.dto.mongodb.TagItemsLog;
...
import com.fosun.data.cleanup.comment.tag.tuple.LogItemsTuple;
...
public class BaiduApiServiceImpl {
     ...
     * @return 获取的记录数
     */
    public int dealDailyCommentInfo(final List<Long> ids,int type){
        Tuple<List<MallCommentInfoPo>, List<ShopCommentInfoPo>> tuple = queryDailyData(ids,type);
        Date startDate =  DateUtil.getTimeOfPreviousDays(0);
        return dealCommentData(tuple,type,startDate,startDate);
    }
     ...
     * @return 获取的记录数
     */
    public int dealHistoryCommentInfo(final List<Long> ids,int type,Date startDate,Date endDate){
        Tuple<List<MallCommentInfoPo>, List<ShopCommentInfoPo>> tuple = queryHistoryData(ids,type,startDate,endDate);
        return dealCommentData(tuple,type,startDate,endDate);
    }

    /**
     * 处理评论
     * @param tuple 商场或店铺的mongodb查询结果信息
     * @param type 商场或店铺类型 0：商场， 1：店铺， 其它值，若ids为空，则同时处理店铺和商场的所有数据
     * @param  startDate 起始日期
     * @param  endDate 结束日期 ，如果为空，则默认为当天
     * @return 获取的记录数
     */
    private int dealCommentData(Tuple<List<MallCommentInfoPo>, List<ShopCommentInfoPo>> tuple,int type,Date startDate,Date endDate){
        if(null == tuple){
            tuple = new Tuple<>(Collections.EMPTY_LIST,Collections.EMPTY_LIST);
        }
        ...
        if( !CollectionUtils.isEmpty(mallCommentInfoPos)){
            ...
            try{
                ...
                log.error("处理商场评论出错..." + e.getMessage());
            }
        }else{
     ...
     */
    @Deprecated
    private List<MongoCommentTagInfoPo> getMongoTagData(final List<MongoCommentTagInfoPo> mongoCommentTagInfoPos,SubjectTypeEnum typeEnum) {
            ...
            if(!CollectionUtils.isEmpty(pos)){
                ...
                for(Map.Entry<String, List<MongoCommentTagInfoPo>> entry: collect.entrySet()){
                    ...
                    if(po == null){
                        ...
                        items.addAll(po.getTag());
                        Update update = new Update();
                        update.set("tag",items);
                        update.set("updateTime",updateTime);
                        Query query = new Query(new Criteria("_id").is(po.getId()));
                        mongoTemplate.updateMulti(query,update,MongoCommentTagInfoPo.class);
                    }
                    cnt++;
                }
     ...
     */
    private void dealShopCommentInfo(final List<ShopCommentInfoPo> shopCommentInfoPos, final List<MongoCommentTagInfoPo> pos)  {
        long start = System.currentTimeMillis();
        Map<Integer, Map<Long, List<ShopCommentInfoPo>>> shopCommentMap = shopCommentInfoPos.stream()
         ...
         shopCommentMap.forEach((commentType,idMap)->idMap.forEach((id,data)->{
            ...
            data.stream().forEach(x->{
                try {
                    LogItemsTuple<List<String>, String,Map<String, TagItemsLog>>  contentTuple = requestTagParse(x.getId(),x.getContent(),eSimnetType);
                    if(!CollectionUtils.isEmpty(contentTuple.t1)){
                        ...
                        contentTuple.t1.stream().forEach(tag ->{
                            ...
                            saveTags(pos,eSimnetType.ordinal(),tag,SubjectTypeEnum.SHOP.getValue().intValue());
                        });
                        updateDocument(x.getId(),contentTuple.t2,contentTuple.t1,contentTuple.t3,1);
                    }
                } catch (Exception e) {
                    // 保存处理失败的信息
     ...
     */
    private void dealMallCommentInfo(final List<MallCommentInfoPo> mallCommentInfoPos ,final List<MongoCommentTagInfoPo> pos) {
        ...
        mallCommentMap.forEach((commentType,idMap)-> idMap.forEach((id, data)->{
            ...
            data.stream().forEach(x->{
                try {
                    LogItemsTuple<List<String>, String,Map<String, TagItemsLog>> contentTuple = requestTagParse(x.getId(),x.getContent(),ESimnetType.SHOPPING);
                    if(!CollectionUtils.isEmpty(contentTuple.t1)){
                        ...
                        updateDocument(x.getId(),contentTuple.t2,contentTuple.t1,contentTuple.t3,0);
                    }
                } catch (Exception e) {
     ...
     * @return 更新的文档数
     */
    private long updateDocument(String id, String highLightContent,List<String> tags,Map<String, TagItemsLog> logTagMap,int type){
        try{
             log.info("update document now....");
            Update update = new Update();
            update.set("commentTag",tags);
            update.set("highlighContent",highLightContent);
            update.set("logTagMap",logTagMap);
    ...
    private void saveTags( final List<MongoCommentTagInfoPo> pos ,int bizCls, String tag,int source){
        ...
        if( !CollectionUtils.isEmpty(pos) ){
            ...
            if(optional.isPresent()){
                MongoCommentTagInfoPo  po = optional.get();
                if(po.getTag() != null){
                    po.getTag().add(tag);
                }else{
            ...
            MongoCommentTagInfoPo po  = new MongoCommentTagInfoPo();
...
//                        }
    ...
    /**
     *
     * 通过url的方式获取百度的解析结果，添加备份处理机制
     * @param content 待解析内容
     * @return 解析的结果
     */
    public JSONObject getAPIData(String content , ESimnetType eSimnetType){
        log.info("enter getAPIData.....");
        JSONObject res = requestApiDataParse(key, secret, content, eSimnetType);
        log.info(res.toJSONString());
        log.info("exit getAPIData.....");
        return res;
    }

    /**
     * 调用百度api
     * @param accKey 访问的 key
     * @param assSecret 访问的密钥
     * @param content 内容
     * @param eSimnetType 类型
     * @return 处理结果
     */
    private JSONObject requestApiDataParse(String accKey,String assSecret ,String content , ESimnetType eSimnetType){
        //获取key
        String token = getAccessKey(accKey,assSecret);
     ...
     * @return tuple 对象 ，属性1：标签集合 属性2： 包含高亮信息评论详情
     */
    private LogItemsTuple<List<String>, String,Map<String, TagItemsLog>> requestTagParse(String id, String content, ESimnetType eSimnetType)  {
        ...
        List<String> tags = new ArrayList<>();
        Map<String, TagItemsLog> tagLogMap = new HashMap<>();
        if(null == res.get("items")){
            log.warn("评论id:{}百度api调用失败",id);
            failureCount.getAndIncrement();
            return new LogItemsTuple(new ArrayList<>(0),content,new HashMap<>(0));
        }
        log.info("调用百度api成功");
        ...
        for(int i = 0; i< arr.size();i++) {
            ...
            //添加源数据
            TagItemsLog tagItemsLog = new TagItemsLog();

            if (StringUtils.isNotBlank(obj.getString("prop"))) {
                tag += obj.getString("prop");
                tagItemsLog.setProp(obj.getString("prop"));
            }
            if (StringUtils.isNotBlank(obj.getString("adj"))) {
                tag += obj.getString("adj");
                tagItemsLog.setAdj(obj.getString("adj"));
            }
            tagItemsLog.setAbs(obj.getString("abstract"));
            tagItemsLog.setSentiment(obj.getInteger("sentiment"));
            tagItemsLog.setBeginPos(obj.getInteger("begin_pos"));
            tagItemsLog.setEndPos(obj.getInteger("end_pos"));

            if (StringUtils.isNotBlank(tag)) {
                tags.add(tag);
                //和标签list一一对应 map
                tagLogMap.put(tag,tagItemsLog);
            }
          ...
          return new LogItemsTuple<>(tags,content,tagLogMap);
     ...
     * @return 店铺和商场评论列表
     */
    private Tuple<List<MallCommentInfoPo>, List<ShopCommentInfoPo>> queryHistoryData(final List<Long> ids,int type, Date startDate,Date endDate){
        Date startTime = startDate == null ? DateUtil.getTimeOfPreviousDays(MINUS_DAYS) : startDate;
        Criteria criteria = new Criteria();
        if(endDate != null){
            criteria.and("createTime").gte(startTime).lt(endDate);
        }else{
            criteria.and("createTime").gte(startTime);
        }
        criteria.and("commentTag").exists(false);
        log.info("query mongodb data, start date:{} ,endDate:{}",startDate,endDate);
        return queryMongoData(criteria,ids,type);
    }


    /**
     * 从mongodb 中查询每日新增的评论数据
     * @param ids  待查询的id .
     * @param type 类型 0：商场， 1：店铺 ，其它值:若ids为空，则同时处理店铺和商场的所有数据

     * @return 店铺和商场评论列表
     */
    private Tuple<List<MallCommentInfoPo>, List<ShopCommentInfoPo>> queryDailyData(final List<Long> ids,int type) {
        Date startTime =  DateUtil.getTimeOfPreviousDays(0);
        Criteria criteria = new Criteria();
        criteria.and("createTime").gte(startTime);
        criteria.and("commentTag").exists(false);
        criteria.and("content").ne("");
        log.info("查询当天的增量评论数据, 起始日期:{}",startTime);
        return queryMongoData(criteria,ids,type);
    }

    /**
     * 查询mongodb 数据
     * @param criteria 查询的条件类
     * @param ids 指定id信息
     * @param type 查询类型
     * @return 查询结果信息
     */
    private Tuple<List<MallCommentInfoPo>, List<ShopCommentInfoPo>> queryMongoData(Criteria criteria,final List<Long> ids,int type){
        if(null == criteria){
    ...
    public int runMakeupData(Integer type, String strStartDate, String strEndDate) {
        ...
        try{
            ...
            if(start.after(end)){
                ...
                num = dealHistoryCommentInfo(null,type,start,end);
            }
