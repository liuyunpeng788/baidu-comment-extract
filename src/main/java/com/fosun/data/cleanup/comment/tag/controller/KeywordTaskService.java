...
public class KeywordTaskService implements Runnable {
    ...
    public void run() {
         if(StringUtils.isNotBlank(strEndDate) && StringUtils.isNotBlank(strStartDate) && null != type){
            //走补数据的流程
            log.info("开始处理补数据流程,type:{},start:{},end:{}",type,strStartDate,strEndDate);
            baiduApiService.runMakeupData(type,strStartDate,strEndDate);
            log.info("开始处理补数据流程结束,type:{},start:{},end:{}",type,strStartDate,strEndDate);
             return;
        }
        ...
        if(!ignoreTodayFlag){
            log.info("开始处理当天增量评论数据....");
            long begin = System.currentTimeMillis();
            //处理当天的数据
            try {
                baiduApiService.dealDailyCommentInfo(null,type);
            } catch (Exception e) {
                log.error("处理当天的评论数据出现异常.错误信息:{}",e.getMessage() );
            }
            log.info("处理当天增量评论数据结束, 耗时:{}s" ,(System.currentTimeMillis()-begin)*1.0/1000 );
        }
        ...
        while(times < THRESHOLD && localDateTime.getHour()< STOP_HOUR){
            ...
            try {
                int records = baiduApiService.dealHistoryCommentInfo(null,type,start,end);
                if(records == 0){
